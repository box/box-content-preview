	  ...
	  _createClass(BaseRepresentationLoader, [{
	    ...
	    */
	    ...
	    value: function getRepresentation(url, progress) {
	      ...
	      return new _lie2['default'](function (resolve, reject) {
	        _this2.get(url, params, progress).then(function (xhr) {
	          switch (xhr.status) {
	            case 200:
	              // found, yay
	              resolve(xhr);
	    ...
	    */
	    ...
	    value: function buildRepresentationUrl(fileId, fileVersionId, repParams) {
	      ...
	      if (repParams.properties) {
	        props = '?' + this.xhr.encodeToUri(repParams.properties);
	      }
	
	      if (!this.contentBaseCache[fileId]) {
	
	        var repInfoUrl = this.apiBase + '/2.0/files/' + fileId + '?fields=representations';
	        // From here, we're going to get a content base url for representations
	        this.contentBaseCache[fileId] = this.xhr.get(repInfoUrl, null, { responseType: 'json' }).then(function (resp) {
...
/***/ }
/******/ ]);
//# sourceMappingURL=boxsdk-0.2.0.js.map