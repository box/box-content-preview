import Image from '../image';
import fetchMock from 'fetch-mock';

let sandbox = sinon.sandbox.create();
let imageUrl = 'data:image/png;base64,';
let image;

/**
 * Creates and returns a blob from a base64 data URL
 *
 * @private
 * @param {String} dataURL The data URL to convert.
 * @returns {Blob} A blob representing the array buffer data.
 */
function dataURLToBlob(dataURL) {
    const parts = dataURL.split(';base64,');
    const contentType = parts[0].split(':')[1];
    const raw = window.atob(parts[1]);
    const rawLength = raw.length;

    let uInt8Array = new Uint8Array(rawLength);

    for (let i = 0; i < rawLength; ++i) {
        uInt8Array[i] = raw.charCodeAt(i);
    }

    return new Blob([uInt8Array], {
        type: contentType
    });
}

describe('image.js', function() {

    before(function() {
        fixture.setBase('src/lib');
    });

    beforeEach(function() {
        fixture.load('image/__tests__/image-test.html');
        image = new Image('.container');
    });

    afterEach(function() {
        sandbox.verifyAndRestore();
        fixture.cleanup();
        fetchMock.restore();
    });

    it('should load an image and fire load event', function(done) {

        // Mock out fetch and return a blobed image
        fetchMock.mock(imageUrl, {
            body: dataURLToBlob(imageUrl),
            sendAsJson: false
        });

        // Detect load event
        image.on('load', function() {
            expect(fetchMock.called(imageUrl)).to.be.true;
            done();
        });

        // load the image
        image.load(imageUrl);
    });

    it('should not load an image and fire error event', function(done) {

        // Give a timeout larger than 15s
        this.timeout(16000);

        // Return 404 from fetch
        fetchMock.mock('foo', 404);

        // Detect error event due to timeout
        image.on('error', function() {
            expect(fetchMock.called('foo')).to.be.true;
            done();
        });

        // load garbage
        image.load('foo');
    });

    it('should zoom in', function(done) {
        // Mock out fetch and return a blobed image
        fetchMock.mock(imageUrl, {
            body: dataURLToBlob(imageUrl),
            sendAsJson: false
        });

        // Detect load event
        image.on('load', function() {
            const imageEl = document.querySelector('.box-preview-image img');
            const origImageSize = imageEl.getBoundingClientRect();
            image.zoomIn();
            const newImageSize = imageEl.getBoundingClientRect();
            assert.ok(newImageSize.width > origImageSize.width);
            done();
        });

        // load the image
        image.load(imageUrl);
    });

    it('should zoom out', function(done) {
        // Mock out fetch and return a blobed image
        fetchMock.mock(imageUrl, {
            body: dataURLToBlob(imageUrl),
            sendAsJson: false
        });

        // Detect load event
        image.on('load', function() {
            const imageEl = document.querySelector('.box-preview-image img');
            image.zoomIn();
            image.zoomIn();
            const origImageSize = imageEl.getBoundingClientRect();
            image.zoomOut();
            const newImageSize = imageEl.getBoundingClientRect();
            assert.ok(newImageSize.width < origImageSize.width);
            done();
        });

        // load the image
        image.load(imageUrl);
    });
});
