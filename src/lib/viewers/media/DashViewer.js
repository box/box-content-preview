...
class DashViewer extends VideoBaseViewer {
     ...
     *
     * @public
     * @param {Object} [skillsData] - optional skills data used to generate captions
     * @return {void}
     */
    loadAutoGeneratedCaptions(skillsData) {
        const skillsCards = skillsData || getProp(this.options, 'file.metadata.global.boxSkillsCards.cards');
        // Do nothing if we don't have skills data
        if (!skillsCards) {
            return;
        }

        // Do nothing if we already have captions and the passed in data has not changeds
        if (this.autoCaptionDisplayer && (!this.skillsData || skillsData === this.skillsData)) {
            return;
        }

        // Save passed in skills data to avoid regenerating captions unnecessarily
