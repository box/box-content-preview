...
describe('lib/viewers/media/DashViewer', () => {
    ...
    describe('loadeddataHandler()', () => {
        ...
        test('should load the metadata for the media element, show the media/play button, load subs, check for autoplay, and set focus', () => {
            ...
            expect(dash.loadTranscription).toBeCalled();
    ...
    describe('loadTranscription()', () => {
                ...
                file: {
                    representations: {
                        entries: [
                            ...
                            {
                                representation: 'extracted_text_eng',
                                content: { url_template: 'https://api.box.com/transcription-eng.vtt' },
                                status: { state: 'success' },
                            },
                        ],
                    },
        ...
        test('should load extracted_text_eng when extracted_text is unavailable', done => {
            ...
            const mockAddTrackPromise = Promise.resolve();
            ...
            mockRepStatusPromise
                .then(() => mockAddTrackPromise)
                .then(() => {
        ...
        test('should create RepStatuses and add both transcription tracks when ready', done => {
            const mockRepStatusPromise = Promise.resolve();
            const mockAddTrackPromise = Promise.resolve();
            ...
            mockRepStatusPromise
                ...
                .then(() => mockAddTrackPromise)
                .then(() => {
                    expect(dash.player.addTextTrackAsync).toHaveBeenCalledTimes(2);
        ...
        test('should re-render UI without resetting selection when React controls and tracks already exist', done => {
            ...
            dash.player.addTextTrackAsync = jest.fn().mockReturnValue(mockAddTrackPromise);
            dash.player.getTextTracks = jest.fn().mockReturnValue([{ id: 1, language: 'eng' }, transcriptionTrack]);
            dash.textTracks = [{ id: 1, language: 'eng' }];
        ...
        test('should preserve Auto-Generated label for existing und tracks in React controls', done => {
            dash.options.file.representations.entries = [dash.options.file.representations.entries[1]];
            const mockRepStatusPromise = Promise.resolve();
            const mockAddTrackPromise = Promise.resolve();
            const transcriptionTrack = { id: 99, language: 'und', label: 'Transcription' };
            ...
            dash.player.addTextTrackAsync = jest.fn().mockReturnValue(mockAddTrackPromise);
            dash.player.getTextTracks = jest
                .fn()
                .mockReturnValue([{ id: 1, language: 'und' }, { id: 2, language: 'eng' }, transcriptionTrack]);
            dash.textTracks = [
                { id: 1, language: 'und' },
                { id: 2, language: 'eng' },
            ];
        ...
        test('should append only new tracks to settings when non-React controls and tracks already exist', done => {
            dash.options.file.representations.entries = [dash.options.file.representations.entries[1]];
            const mockRepStatusPromise = Promise.resolve();
            const mockAddTrackPromise = Promise.resolve();
            ...
            dash.mediaControls.settings = { addSubtitle };
        ...
        test('should handle transcription load failure gracefully', async () => {
            dash.options.file.representations.entries = [dash.options.file.representations.entries[1]];
            const mockRepStatusPromise = Promise.reject(new Error('conversion failed'));
        ...
        test('should use createContentUrlV2 when migrateAccessTokenToHeader is enabled', async () => {
            ...
            expect(createUrlV2).toHaveBeenCalledWith('https://api.box.com/transcription-eng.vtt');
            expect(stubs.createUrl).not.toBeCalled();
        });
        ...
        test('should use createContentUrlWithAuthParams when migrateAccessTokenToHeader is disabled', async () => {
            ...
            expect(stubs.createUrl).toHaveBeenCalledWith('https://api.box.com/transcription-eng.vtt');
        });
    });
    ...
    describe('loadSubtitles()', () => {
        ...
        test('should map und to Auto-Generated', () => {
    ...
    describe('initSubtitles()', () => {
        beforeEach(() => {
            ...
            const english = { language: 'eng', id: 5 };
            const russian = { language: 'rus', id: 4 };
            const spanish = { language: 'es', id: 6 };
            const korean = { language: 'kor', id: 3 };
            const chinese = { language: 'zho', id: 7 };
            const subs = [english, russian, spanish, korean, chinese];

            dash.textTracks = subs;
        });

        test('should add displayLanguage to the textTracks', () => {
            dash.initSubtitles();

            const displayLanguages = ['English', 'Russian', 'Spanish', 'Korean', 'Chinese'];
            dash.textTracks.forEach(({ displayLanguage }, index) => {
                expect(displayLanguage).toBe(displayLanguages[index]);
            });
        });

        test('should prefer the client locale language for subtitles', () => {
            dash.options.location = { locale: 'es' };

            dash.initSubtitles();

            expect(dash.selectedSubtitle).toBe(6);
        });

        test('should fall back to English for subtitles', () => {
            dash.initSubtitles();

            expect(dash.selectedSubtitle).toBe(5);
        });

        test('should fall back to the first track for subtitles', () => {
            const russian = { language: 'rus', id: 4 };
            const spanish = { language: 'es', id: 6 };
            const korean = { language: 'kor', id: 3 };
            const chinese = { language: 'zho', id: 7 };
    ...
    describe('setSubtitle()', () => {
        ...
        test('should select track from front of text track list', () => {
            ...
            stubs.mockPlayer.expects('selectTextTrack').withArgs({ language: 'eng', id: 5 });
            stubs.mockPlayer.expects('setTextTrackVisibility').withArgs(true);
        ...
        test('should select track from end of text track list', () => {
            ...
            stubs.mockPlayer.expects('selectTextTrack').withArgs({ language: 'zho', id: 7 });
            stubs.mockPlayer.expects('setTextTrackVisibility').withArgs(true);
        ...
        test('should select track from middle of text track list', () => {
            ...
            stubs.mockPlayer.expects('selectTextTrack').withArgs({ language: 'es', id: 6 });
            stubs.mockPlayer.expects('setTextTrackVisibility').withArgs(true);
        ...
        test('should not set the subtitle ID to cache when turning off', () => {
            ...
            expect(dash.cache.set).toBeCalledWith('media-subtitles-toggle', false, true);
            expect(dash.cache.set).not.toBeCalledWith('media-subtitles');
        });

        test('should set the subtitle ID to cache when not turning off', () => {
            dash.setSubtitle(5);

            expect(dash.cache.set).toBeCalledWith('media-subtitles-toggle', true, true);
            expect(dash.cache.set).toBeCalledWith('media-subtitles', 5, true);
        });
    });
