...
| comment_range_draft_change  | A draft range handle drag finished (audio player v2). Pointer-up only.                                              | `{ startMs, endMs }` milliseconds; `endMs > startMs`                                                                                                                           |
| comment_range_draft_dismiss | Viewer → host: the user clicked the waveform outside an open draft range. Host should uncheck the timestamp toggle. |                                                                                                                                                                                |

The host also sends these events to the MP3 viewer (audio player v2 only). Nothing is shown until the first `comment_range_draft`.
