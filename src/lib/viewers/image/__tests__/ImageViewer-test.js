...
describe('lib/viewers/image/ImageViewer', () => {
    ...
    describe('createAnnotator()', () => {
        ...
        beforeEach(() => {
            initStub = sandbox.stub(ImageAnnotator.prototype, 'init');
        });

        it('should create an instance of an ImageAnnotator', () => {
            image.createAnnotator();
            expect(image.annotator).to.be.an.instanceof(ImageAnnotator);
            expect(initStub).to.be.called;
        });

        it('should add an event listener for Point Mode Enter, on the annotator', () => {
            const enterStub = sandbox.stub(ImageAnnotator.prototype, 'addListener');
            image.createAnnotator();
