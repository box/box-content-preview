...
class PreviewUI {
    ...
    setupHeader(headerTheme, logoUrl) {
        ...
        // Set custom logo
        if (logoUrl) {
            const defaultLogoEl = headerEl.querySelector(
                SELECTOR_BOX_PREVIEW_LOGO_DEFAULT
            );
            defaultLogoEl.classList.add(CLASS_HIDDEN);

            const customLogoEl = headerEl.querySelector(
                SELECTOR_BOX_PREVIEW_LOGO_CUSTOM
            );
     ...
     */
    setupLoading() {
        const loadingWrapperEl = this.container.querySelector(
            SELECTOR_BOX_PREVIEW_LOADING_WRAPPER
        );
        if (!loadingWrapperEl) {
            return;
        ...
        const iconWrapperEl = loadingWrapperEl.querySelector(
            SELECTOR_BOX_PREVIEW_ICON
        );
        iconWrapperEl.innerHTML = ICON_FILE_DEFAULT;

        const loadingTextEl = loadingWrapperEl.querySelector(
            SELECTOR_BOX_PREVIEW_LOADING_TEXT
        );
        loadingTextEl.textContent = __('generating_preview');

        const loadingDownloadButtonEl = loadingWrapperEl.querySelector(
            SELECTOR_BOX_PREVIEW_BTN_LOADING_DOWNLOAD
        );
    ...
    /**
     * Destroy preview this.container content.
     *
     * @return {void}
     */
    cleanup() {
    ...
    /**
     * Initializes the this.container for preview.
     *
     ...
     * @return {HTMLElement} Preview this.container
     */
    setup(options, keydown, navigateLeft, navigateRight, mousemove) {
        ...
        if (typeof this.container === 'string') {
            // Get the this.container dom element if a selector was passed instead.
            this.container = document.querySelector(this.container);
        } else if (!this.container) {
            // Create the this.container if nothing was passed.
        ...
        // Create the preview with absolute positioning inside a relative positioned this.container
        // <bp-this.container>
        //      <bp-header>
        //      <bp>
        //      <navigation>
    ...
    showNavigation(id, collection) {
        ...
        const leftNavEl = this.container.querySelector(
            SELECTOR_NAVIGATION_LEFT
        );
        const rightNavEl = this.container.querySelector(
            SELECTOR_NAVIGATION_RIGHT
        );
    ...
    showPrintButton(handler) {
        const printButtonEl = this.container.querySelector(
            SELECTOR_BOX_PREVIEW_BTN_PRINT
        );
        if (!printButtonEl) {
            return;
    ...
    showDownloadButton(handler) {
        const downloadButtonEl = this.container.querySelector(
            SELECTOR_BOX_PREVIEW_BTN_DOWNLOAD
        );
        if (!downloadButtonEl) {
            return;
